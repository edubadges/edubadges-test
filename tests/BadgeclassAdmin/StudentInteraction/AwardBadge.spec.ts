...
institutionsWithoutHBO.forEach((institution) => {
  ...
  test(`Send badge directly from ${institution} and check audit trail`, async ({
    ...
    await adminPage.page
      .locator('div')
      .filter({ hasText: 'DirectAward audit trail .' })
      .nth(3)
      .click();
  ...
  test(`Send badge directly from ${institution} through mail`, async ({
    ...
    await backpackPage.page
      .getByText(
        'Regulation and Integration Issued by Medicine (Medicine) Study load 2.5 ECTS/EC',
      )
      .click();
    await backpackPage.page
      .getByRole('link', { name: 'Claim & Add to your backpack' })
    ...
    await backpackPage.page
      .getByRole('link', { name: 'Copy the link' })
      .click();
    // 2. Retrieve the clioboard content
    const clipboardText = await backpackPage.page.evaluate(() =>
  ...
  test(`Award requested badge from ${institution} with expiration date`, async ({
    ...
    const tomorrowFormattedDate = today.toLocaleDateString(
      'en-US',
      dateOptions,
    );
