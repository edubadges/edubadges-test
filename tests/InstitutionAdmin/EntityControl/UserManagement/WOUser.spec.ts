...
test('Invite WO user', async ({ woInstitutionAdminPage: woPage }) => {
  ...
  // validate
  await expect(
    woPage.page.getByText(`Successfully invited ${newUserMail}`),
  ).toBeVisible();
});

test('Revoke WO user invite', async ({ woInstitutionAdminPage: woPage }) => {
  // var
  ...
  // validate
  await expect(
    woPage.page.getByText('Successfully removed invite'),
  ).toBeVisible();
...
test('Delete WO permission', async ({
  ...
  // validate
  await expect(
    woPage.page.getByText('Successfully removed rights'),
  ).toBeVisible();
